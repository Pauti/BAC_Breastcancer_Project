...
def preprocess_image(image_path, desired_size):
    ...
    # crop edgs of image to remove borders
...
# leave train and test data separate
...
x,y = mass_train['preprocessed_images'].iloc[0], mass_train['pathology'].iloc[0]
plt.imshow(x)
plt.savefig('breastcancer_detection_p3.11/Plots/Preprocessed_Image.png')
plt.show()
...
X_train, X_val, y_train, y_val = train_test_split(train_resized, y_train, test_size=0.2, random_state=42)
...
def cnn_architecture():
    ...
    # Train Model
    history = model.fit(train_datagen_augmented, # augmented data
